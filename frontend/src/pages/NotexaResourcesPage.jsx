...
export default function NotexaResourcesPage() {
  const { id } = useParams();
const selectedSubject = resourcesData.find(
  (item) => item.subjectId === String(id)
);
          ...
          <div className="relative flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
            <div className="max-w-2xl">
              <div className="inline-flex items-center gap-2 rounded-full bg-white/10 px-3 py-1 text-xs font-semibold text-white/90 ring-1 ring-inset ring-white/15">
                ...
                {selectedSubject.subject.category}
              </div>

              <h1 className="mt-3 text-2xl font-bold tracking-tight sm:text-3xl">{selectedSubject.subject.title}</h1>
              <p className="mt-2 text-sm leading-relaxed text-white/80 sm:text-base">{selectedSubject.subject.description}</p>
            </div>
        ...
        <div className="mt-8 grid gap-8">
          <section>
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-2">
              {selectedSubject.notes.map((item) => (
